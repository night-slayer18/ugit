...
from .remote import get_remote_url, get_all_remotes
...
def _fetch_local(repo: Repository, remote_name: str, remote_url: str, branch: Optional[str]) -> None:
    """
    ...
    for branch_name, commit_sha in remote_refs.items():
        ...
        if old_sha != commit_sha:
            ...
            if old_sha:
                ...
                print(f" * [new branch] {remote_name}/{branch_name} -> {remote_name}/{branch_name}")
...
def _get_remote_refs(remote_url: str, branch_filter: Optional[str] = None) -> Dict[str, str]:
    """
