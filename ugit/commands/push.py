...
def push(remote_name: str = "origin", branch: Optional[str] = None, force: bool = False) -> None:
    """
...
def _push_local(repo: Repository, remote_name: str, remote_url: str, branch: str, local_sha: str, force: bool) -> None:
    """
    ...
    if remote_sha:
        if force and not _is_fast_forward(repo, remote_sha, local_sha):
            print(f" + {remote_sha[:8]}...{local_sha[:8]} {branch} -> {branch} (forced update)")
        else:
...
def _collect_objects(sha: str, to_push: Set[str], visited: Set[str], remote_url: str) -> None:
    """
...
def _copy_object_to_remote(local_objects_dir: str, remote_objects_dir: str, sha: str) -> bool:
    """
...
def _is_local_path(url: str) -> bool:
    ...
    Returns:
        True if local path
    """
    return (os.path.isabs(url) or 
            url.startswith('./') or 
            url.startswith('../') or
            (not url.startswith(('http://', 'https://', 'git://', 'ssh://')) and '@' not in url))